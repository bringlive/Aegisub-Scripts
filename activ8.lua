...
function modifire(subs,sel)
	...
	{x=5,y=hbox+1,width=4,class="checkbox",name="kore",label="&Korelator"},
	...
	if P=="▽" then ak() end
	if res.kore then P="Korelator" end
	...
	if P=="Korelator" then
		rezk=rezk or {}
		local G={
		{x=0,y=0,class="label",label="If"},
		{x=1,y=0,class="dropdown",name='tag1',value=rezk.tag1 or 'pos x',hint='tag1',
			items={'pos x','pos y','fscx','fscy','fax','fay','frx','fry','frz','xshad','yshad','shad','bord','blur','fsp'}},
		{x=2,y=0,class="label",label="=="},
		{x=2,y=1,class="label",label="=="},
		{x=3,y=0,class="floatedit",name='t1a',value=rezk.t1a,hint='value A1 (lower)'},
		{x=3,y=1,class="floatedit",name='t1b',value=rezk.t1b,hint='value B1 (higher)'},
		{x=4,y=0,class="label",label="then"},
		{x=5,y=0,class="dropdown",name='tag2',value=rezk.tag2 or 'fscx',hint='tag2',
			items={'pos x','pos y','fscx','fscy','fax','fay','frx','fry','frz','xshad','yshad','shad','bord','blur','fsp'}},
		{x=6,y=0,class="label",label="="},
		{x=6,y=1,class="label",label="="},
		{x=7,y=0,class="floatedit",name='t2a',value=rezk.t2a,hint='value A2'},
		{x=7,y=1,class="floatedit",name='t2b',value=rezk.t2b,hint='value B2'},
		{x=0,y=2,width=2,class="label",label="Out of range:"},
		{x=2,y=2,width=2,class="dropdown",name='oor',value=rezk.oor or 'linear interpolation',hint='what to do with values <A1 or >B1',
			items={'linear interpolation','keep limits (A2 & B2)','mirror around A1','mirror around B1'}},
		{x=4,y=2,width=3,class="floatedit",name='vmin'},
		{x=7,y=2,class="floatedit",name='vmax'},
		{x=0,y=3,width=7,class="label",label="This applies to start tags that aren't in transforms."},
		}
		repeat
			if KP=="&Check tag1 values" then
				tag1=rezk.tag1
				vmin=999999
				vmax=-999999
				for z,i in ipairs(sel) do
					local l=subs[i]
					local stags=l.text:match"^{>?\\[^}]-}" or ""
					local t=detra(stags)
					if tag1=='pos x' then vv=t:match("\\pos%(([^,]+)")
					elseif tag1=='pos y' then vv=t:match("\\pos%([^,]+,([^)]+)")
					else vv=t:match("\\"..tag1.."([%d.-]+)")
					end
					vvv=tonumber(vv)
					if vvv then
						if vvv>vmax then vmax=vvv end
						if vvv<vmin then vmin=vvv end
					end
				end
				if vmin==999999 or vmax==-999999 then
					t_error("No '"..tag1.."' tags found in selected lines.")
					vmin=0 vmax=0
				end
				for k,v in ipairs(G) do
					if v.name=='vmin' then v.value=vmin v.hint='Min. '..rezk.tag1..' value found in selected lines'
					elseif v.name=='vmax' then v.value=vmax v.hint='Max. '..rezk.tag1..' value found in selected lines'
					else v.value=rezk[v.name]
					end
				end
			end
			KP,rezk=ADD(G,{"&Korelate","&Check tag1 values","▽"},{ok='&Korelate',close='▽'})
			if KP=="▽" then ak() end
			tag1=rezk.tag1
			tag2=rezk.tag2
			A1=rezk.t1a
			...
			A2=rezk.t2a
			B2=rezk.t2b
			if KP=="&Korelate" and (A1==B1 or A2==B2) then t_error("B1 must be different from A1.\nB2 must be different from A2.") KP='' end
		until KP=="&Korelate"
		step=(B2-A2)/(B1-A1)
		noneg="\\bord\\shad\\xbord\\ybord\\fs\\blur\\be\\fscx\\fscy"
		if rezk.oor:match'mirror' and A1>B1 then t_error("A1 is higher than B1. This will produce wrong results with mirroring.") end
	end
	...
	for z,i in ipairs(sel) do
		...
		if P=="Korelator" then
			stags=t:match"^{>?\\[^}]-}" or ""
			dt=detra(stags)
			if tag1=='pos x' then V1=dt:match("\\pos%(([^,]+)")
			elseif tag1=='pos y' then V1=dt:match("\\pos%([^,]+,([^)]+)")
			else V1=dt:match("\\"..tag1.."([%d.-]+)")
			end
			if V1 then
				if rezk.oor:match'A1' and tonumber(V1)<A1 then V1=A1+(A1-V1) end
				if rezk.oor:match'B1' and tonumber(V1)>B1 then V1=B1-(V1-B1) end
				V2=round((V1-A1)*step+A2,2)
				if rezk.oor:match'limits' then
					if A2<B2 then s2=A2 l2=B2 else s2=B2 l2=A2 end
					if V2<s2 then V2=s2 end
					if V2>l2 then V2=l2 end
				end
				if tonumber(V2)<0 and noneg:match('\\'..tag2) then V2='0' end
				if tag2=='pos x' then t=t:gsub("\\pos%([^,]+","\\pos("..V2)
				elseif tag2=='pos y' then t=t:gsub("(\\pos%([^,]+,)[^)]+","%1"..V2)
				else t=addtag3('\\'..tag2..V2,t)
				end
			end
		end
		l.text=t
