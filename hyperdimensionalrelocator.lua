...
function relocator(subs,sel,act)
	...
	Bilocator={"transmove","horizontal","vertical","multimove","clip2move","rvrs. move","shiftstart","shiftmove","move to","move clip","randomove","kill times","full times","set times","overlap","zig-zag..."}
	Morphing={"round numbers","line2fbf","join fbf lines","move v. clip","set origin","calculate origin","transform clip","set rotation","rotate 180","negative rot","spin doctor","vector2rect.","rect.2vector","clip scale","clip2scale fbf","find centre","extend mask","flip mask","expand clip...","adjust drawing","randomask","randomise...","letterbreak","wordbreak","clip info","[un]hide clip"}
...
function positron(subs,sel)
	...
	if res.posi=="Align X" then
		...
		if z==1 and res.first then
			...
			if not res.first then px=ps end
	...
	if res.posi=="Align Y" then
		...
		if z==1 and res.first then
			...
			if not res.first then py=ps end
			if nopos2 then text=getpos(subs,text) end
...
function bilocator(subs,sel)
	...
	if rM=="overlap" and i<#subs and not text:match('\\an[456789]') and (line.style:match"Defa" or line.style:match"Alt" or line.style:match"Overlap") and not line.comment then
		l2=subs[i+1]
		t2=l2.text
		if l2.start_time<line.end_time and not t2:match('\\an[456789]') and (l2.style:match"Defa" or l2.style:match"Alt" or l2.style:match"Overlap") and not l2.comment then
			if nopos then text=getpos(subs,text) end
			sr=stylechk(subs,line.style)
			local intrvl=l2.start_time-line.start_time-res.post
			local shift=300
			local fs=sr.fontsize
			if res.force>0 then shift=res.force end
			text=text:gsub("pos%(([^,]+),([^,]+)%)",function(x,y)
				mc=x..','..y..','..x..','..y-fs
				tc1=intrvl-shift
				if tc1<0 then tc1=0 end
				tc=','..tc1..','..intrvl
				return 'move('..mc..tc..')'
				end)
		end
...
function modifier(subs,sel,act)
	...
	if res.mod=="expand clip..." then
		if z==1 then
			CR=CR or {}
			if CR.CTR==nil then CR.CTR=true end
			if CR.CTT==nil then CR.CTT=true end
			local G={
			{x=0,y=0,width=4,class="label",label="Expand rectangular clips in any direction (px)"},
			{x=0,y=1,class="label",label=":::::::::::::"},
			{x=3,y=1,class="label",label=":::::::::::::"},
			{x=1,y=1,width=2,class="floatedit",name="CT",hint='top',value=CR.CT},
			{x=0,y=2,width=2,class="floatedit",name="CL",hint='left',value=CR.CL},
			{x=2,y=2,width=2,class="floatedit",name="CR",hint='right',value=CR.CR},
			{x=1,y=3,width=2,class="floatedit",name="CB",hint='bottom',value=CR.CB},
			{x=0,y=3,class="label",label=":::::::::::::"},
			{x=3,y=3,class="label",label=":::::::::::::"},
			{x=0,y=4,width=2,class="checkbox",name="CTR",label="&Regular clips",value=CR.CTR},
			{x=2,y=4,width=2,class="checkbox",name="CTT",label="Clips in &transforms",value=CR.CTT},
			{x=0,y=5,width=4,class="checkbox",name="usetop",label="Use top value for &all 4",value=CR.usetop,hint="because you're lazy and don't wanna type 4 numbers"},
			}
			CP,CR=ADD(G,{"Expand Rectangular Clip","Esc"},{ok='Expand Rectangular Clip',close='Esc'})
			if CP=="Esc" then ak() end
			if CR.usetop then CR.CL,CR.CR,CR.CB=CR.CT,CR.CT,CR.CT end
		end
		text=text:gsub("\\t(%b())",function(t) return "\\tra"..t:gsub("\\","/") end)
		if CR.CTR then
			text=text:gsub('\\clip%(([^,]+),([^,]+),([^,]+),([^,]-)%)',function(a,b,c,d)
			return '\\clip'..par({a-CR.CL,b-CR.CT,c+CR.CR,d+CR.CB}) end)
		end
		if CR.CTT then
			text=text:gsub('/clip%(([^,]+),([^,]+),([^,]+),([^,]-)%)',function(a,b,c,d)
			return '/clip'..par({a-CR.CL,b-CR.CT,c+CR.CR,d+CR.CB}) end)
...
function teleport(subs,sel)
    ...
    if res.warp then
	telemod={
	{x=0,y=0,class="label",label=" Warped Teleportation"},
	{x=0,y=1,class="floatedit",name="eggs",hint="X"},
	{x=0,y=2,class="floatedit",name="why",hint="Y"},
	{x=1,y=0,width=2,class="checkbox",name="fram",label="by frame, not by line",hint="by frame instead of by line - for layered signs\ndisabled for 'warp by effect'"},
	{x=1,y=1,class="checkbox",name="wx",label="warp by",hint="warp by line's effect* value instead of line count"},
	{x=1,y=2,class="checkbox",name="wy",label="warp by",hint="warp by line's effect* value instead of line count"},
	{x=2,y=1,class="dropdown",name="xf",value="effect",items={"actor","effect","margin_l","margin_r","margin_t"},hint="*use value from a given field"},
	{x=2,y=2,class="dropdown",name="yf",value="effect",items={"actor","effect","margin_l","margin_r","margin_t"},hint="*use value from a given field"},
	{x=0,y=3,width=3,height=4,class="textbox",value="X, Y values = adds that with each line\nwarp by effect = adds X*value_in_effect\n\n[X: 0.5 & effect: '05' -> adds 2.5 for the line]\n[if effect not a number -> adds 0]"},
	}
	tP,rez=ADD(telemod,{"Warped Teleport","Dissolve in Hyperspace"},{ok='Warped Teleport',close='Dissolve in Hyperspace'})
	if tP=="Dissolve in Hyperspace" then ak() end
...
Overlap: For Default-style lines that end after the next line starts, this creates \move to get out of the way.
   disPosition field: ms to end the move before next line begins (0 = move ends as next line appears.)
   Force field: duration of the \move (0 or lower defaults to 300. Both can just be left at 0.)
...
Warped Teleport adds an additional value for each new line.]]
...
stg_port={x=0,y=1,width=2,height=10,class="textbox",name="gd",value=port}
